 ...
 console.log('Received request with authorization code:', code);
  ...
  try {
   ...
   console.log('Token parameters:', tokenParam);
    ...
    if (!dbuser) {
      dbuser = new User({ email:email, name:name, refreshToken:refreshtoken }); // Create a new user if they don't exist
    } else {
...
app.post('/schedule-event/:email', auth, async (req, res) => {
  try {
    ...
    if (err.code === 401 && err.response.data.error_description.includes('Token has expired')) {
